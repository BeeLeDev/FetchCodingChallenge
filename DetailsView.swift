...
struct DetailsView: View {
    @State private var mealDetails = MealDetails(idMeal: "", strMeal: "", strMealThumb: "", strInstructions: "")
    var id: String
                ...
                ForEach(ingredients, id: \.self) { ingredient in
                    if !(ingredient == "") {
                        if let index = ingredients.firstIndex(of: ingredient) {
                            let text = measurements.count > 0 && measurements.indices.contains(index) ? measurements[index] : "Measurement not found"
        ...
        do {
            ...
            if let decodedResponse = try? JSONDecoder().decode(MealDetailsResponse.self, from: data) {
                for meal in decodedResponse.meals {
                    ...
                    print(ingredients)
                    print(measurements)
                    print("Successfully loaded ID: \(id)")
                }
            }
        } catch {
        print("Invalid data")
